...
import dynamic from 'next/dynamic'; // or use React.lazy if using Vite

// Lazy load heavy components
const StickerPicker = dynamic(() => import('./StickerPicker'));
const EmojiPickerButton = memo(dynamic(() => import('./EmojiPicker')));
...
const MessageInput = () => {
  ...
  return (
    <div className="p-3 sm:p-4 w-full bg-base-100 shadow-lg border-t border-zinc-700 relative">
      ...
      {isStickerPickerOpen && <StickerPicker onSelect={handleStickerSelect} />}
    </div>
  );
};
